...
export function UploadManagerClient({ initialCatalog }: { initialCatalog: CatalogDepartment[] }) {
  const router = useRouter();
  const [isUploading, setIsUploading] = useState(false);
  const [message, setMessage] = useState("");
  const today = new Date().toISOString().slice(0, 10);
  const [catalog] = useState<CatalogDepartment[]>(initialCatalog);
  const [departmentName, setDepartmentName] = useState("");
  const [programName, setProgramName] = useState("");
  const [subjectValue, setSubjectValue] = useState("");

  const catalogError = catalog.length
    ? ""
    : "We could not load departments, programs, and subjects from the backend.";
  const catalogReady = Boolean(catalog.length) && !catalogError;

  useEffect(() => {
    const firstDepartment = initialCatalog[0];
        ...
        <CardContent>
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              >
                <option value="">{catalogReady ? "Department" : "Catalog unavailable"}</option>
                {catalog.map((item) => (
            ...
            <Button type="submit" disabled={isUploading || !catalogReady}>
